jQuery(document).ready(function ($) {
    ...
    function logCurrentData(slider, newPosition) {
      ...
      const dataAttribute = slider.data('intervals');
      //var intervals = JSON.parse(dataAttribute);

      // Loop through the intervals array
      dataAttribute.forEach(function(interval) {
          const min_interval = parseInt(interval.min_interval);
          const max_interval = parseInt(interval.max_interval);


          if(max_interval >= currentValue && min_interval <= currentValue){
            slider.find('.offer-name').html(interval.interval_title)
            slider.find('.offer-cartridge').html(interval.interval_label)
            slider.find('.js-output').html(interval.radio_text)
            slider.find('.offer-price').html(interval.pricing)
            slider.find('.txt-black').html(interval.pricing_top_text)
            slider.find('.description').html(interval.interval_description)
            slider.find('.offer_cta').html(interval.button_text)
            slider.find('.offer_cta').attr("href", interval.button_link.url);
          }
